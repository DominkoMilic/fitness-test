...
export function EditFavModal({ onSaved }: { onSaved?: () => void }) {
  ...
  const onConfirm = async () => {
    ...
    try {
      await updateFavorite(payload.fav.id, user?.id, {
        name: name.trim(),
        meal,
        items: outItems,
      ...
      closeModal();
      showToast("Omiljeni obrok ažuriran");
      onSaved?.();
    } catch (error) {
      const message =
      ...
      showToast(message);
      console.error("EditFav save error", error);
    } finally {
      setSaving(false);
    }
  };
