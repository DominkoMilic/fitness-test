...
export async function listLogs(code: string, date: string): Promise<FoodLogRow[]> {
...
export async function getLog(id: string): Promise<FoodLogRow | null> {
  const { data } = await supabase.from("food_logs").select("*").eq("id", id).limit(1);
  return data?.[0] ?? null;
}
...
export async function insertLogs(entries: FoodLogInsert[]) {
  ...
  const { data, error } = await supabase.from("food_logs").insert(entries).select();
  if (error) throw error;
  return data ?? [];
}
